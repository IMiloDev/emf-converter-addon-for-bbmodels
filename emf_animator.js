...
// Optional remote about (NOT used directly by Blockbench UI safely)
fetch("https://raw.githubusercontent.com/IMiloDev/emf-converter-addon-for-bbmodels/main/About.txt")
  .then(r => r.text())
  .then(text => {
    ABOUT_TEXT = text; // usable internally (logs, updates, etc.)
  })
  .catch(() => {
...
function valorNumero(valor) {
  // Igual que _valor_numero() en Python:
  // normaliza strings, null y undefined a un número float.
...
function fitFourier(t, values, period, nHarmonics = N_HARMONICS) {
  ...
  // Resolver el sistema con eliminación gaussiana
